...
<div class="mt-3 border border-[#E5E3DB] bg-white">
    <div class="flex items-start justify-between border-b border-[#E5E3DB] bg-[#FCFBF8] px-8 py-6">
        <div>
            ...
            <h1 class="font-display text-3xl font-semibold text-[#16213A]">{{ $schoolClass['name'] }}</h1>
            <p class="mt-1 font-mono text-xs text-slate-500">Tingkat {{ $schoolClass['grade'] }} — {{ $schoolClass['major'] }}</p>
        </div>
        <a href="{{ route('classes.edit', $schoolClass['id']) }}" class="bg-[#16213A] px-5 py-2.5 text-sm font-medium text-white transition hover:bg-[#26324f]">Ubah</a>
    </div>

    <dl class="divide-y divide-[#EFEDE6] text-sm">
        <div class="flex justify-between px-8 py-4">
            <dt class="uppercase tracking-[0.1em] text-xs text-slate-400">Nama Kelas</dt>
            <dd class="font-medium text-[#16213A]">{{ $schoolClass['name'] }}</dd>
        </div>
        <div class="flex justify-between px-8 py-4">
            <dt class="uppercase tracking-[0.1em] text-xs text-slate-400">Tingkat</dt>
            <dd class="font-medium text-[#16213A]">{{ $schoolClass['grade'] }}</dd>
        </div>
        <div class="flex justify-between px-8 py-4">
            <dt class="uppercase tracking-[0.1em] text-xs text-slate-400">Jurusan</dt>
            <dd class="font-medium text-[#16213A]">{{ $schoolClass['major'] }}</dd>
        </div>
        <div class="flex justify-between px-8 py-4">
            <dt class="uppercase tracking-[0.1em] text-xs text-slate-400">Wali Kelas</dt>
            <dd class="font-medium text-[#16213A]">{{ $schoolClass['homeroom_teacher'] }}</dd>
        </div>
    </dl>
    ...
    <div class="flex justify-end gap-4 border-t border-[#E5E3DB] px-8 py-5">
        ...
        <form action="{{ route('classes.destroy', $schoolClass['id']) }}" method="POST" onsubmit="return confirm('Hapus data kelas ini?')">
            @csrf
            @method('DELETE')
